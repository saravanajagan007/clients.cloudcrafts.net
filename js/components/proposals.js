...
export function renderProposalBuilder(containerEl) {
  ...
  containerEl.innerHTML = `
    <div class="panel-header" style="margin-bottom: 1.5rem;">
      ...
      <button class="action-btn primary" id="btn-create-proposal-builder">+ Create New Proposal</button>
    </div>
      ...
      ${proposals.map(prop => `
        <div class="glass-panel" style="margin-bottom: 0; position: relative;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            <button class="action-btn btn-sm btn-preview-proposal" data-prop-id="${prop.id}">👁️ Preview Proposal</button>
            <button class="action-btn btn-sm primary btn-esign-proposal" data-prop-id="${prop.id}">✍️ Simulate E-Sign</button>
          </div>
        </div>
      `).join('')}
    </div>
  ...
  // Attach event listeners for preview & e-sign simulation
      ...
      <div style="margin-bottom: 1.5rem;">
        ...
        <div style="border-top: 1px dashed var(--border-glass-bright); padding-top: 1rem; margin-top: 1rem;">
          ...
          <p style="font-size: 0.85rem; color: var(--text-muted);">
            ${prop.status === 'Accepted' ? `✅ Digitally signed by Elena Rostova on ${prop.createdDate}` : '⏳ Awaiting client signature online'}
          </p>
        </div>
      </div>
